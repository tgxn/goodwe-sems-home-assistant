...
- Data is pulled by a single DataUpdateCoordinator in [custom_components/sems_au/**init**.py](../custom_components/sems_au/__init__.py):
  - It calls `SemsApi.getData()` in an executor (the API client is synchronous `requests`).
  - It normalizes the SEMS payload into `SemsData` with `inverters` keyed by serial number and station-level `powerflow` data.
...
- Add new sensors by extending `sensor_options_for_data()` in [custom_components/sems_au/sensor.py](../custom_components/sems_au/sensor.py) with a `value_path` list; this makes the entity data-driven and consistent with existing ones.
...
- Device grouping should use `device_info_for_station()` in [custom_components/sems_au/device.py](../custom_components/sems_au/device.py) so all entities attach to the station device.
- Entity unique IDs are station-scoped: `{station_id}-{inverter_serial}-{field}` for inverter data and `{station_id}-powerflow-{field}` for station powerflow data.
...
- Coordinator data shaping: `SemsDataUpdateCoordinator._async_update_data()` in [custom_components/sems/**init**.py](../custom_components/sems/__init__.py).
